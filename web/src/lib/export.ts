...
import { exceptionId, findingLabel, orderedFindings, runId } from './labels'
...
function exceptionsSheet(run: Run): XLSX.WorkSheet {
  const statuses = loadStatuses(runId(run))
  const rows: Cell[][] = [
    ['ID', 'Exception', 'Status', 'Category code', 'Bucket', 'References', 'Amount', 'Rule', 'Evidence'],
  ]
...
function readMeSheet(run: Run): XLSX.WorkSheet {
  const rows: Cell[][] = [
    ...
    ['Exception statuses are working notes recorded in the preparer\u2019s browser; they'],
    ['track follow-up and are not part of the deterministic reconciliation result.'],
    [''],
