...
function exceptionsSheet(run: Run): XLSX.WorkSheet {
  const rows: Cell[][] = [
    ['ID', 'Exception', 'Category code', 'Bucket', 'References', 'Amount', 'Rule', 'Evidence'],
  ]
...
function readMeSheet(run: Run): XLSX.WorkSheet {
  const rows: Cell[][] = [
    ...
    ['Exceptions', 'Every classified difference, with stable IDs, references, amounts, and evidence.'],
